...
class ConvertSeq:
    def __init__(self, bed_path_dir, dataset, output_dir):
        '''
        This script works on non-permutations of records!
    ...
    def convert(self):
        ...
        for x in [60, 70, 80, 90]: 
            ...
            # for z in [500]:
                ...
                if not os.path.exists(f'{self.output_dir}/{x}_0'):
                    os.makedirs(f'{self.output_dir}/{x}_0')
                
                assert os.path.exists(f'{self.output_dir}'), f'Output directory {self.output_dir} does not exist.'

                bed_path = f'{self.bed_path_dir}/{x}_0/{z}_enhancers.bed'
                ...
                enhancer_region = f'{self.output_dir}/{x}_0/{z}_enhancer_region'

                with open(enhancer_region, 'w') as bed_file:
                    for region in region_list:
                        bed_file.write(f'{region}\n')

                self.intersect(enhancer_region, x, z)
                region_list.clear()

    def intersect(self, enhancer_region, x, z):
        '''
        ...
        # print(type(result))
        # exit()
        with open(f'{self.output_dir}/{x}_0/{z}_intersected_enhancers.bed', 'w') as bed_file:
...
if __name__ == '__main__':
    ...
    # Example run: 
    # python3 generate_overlap_seq.py Out_12_12_2023 ../Data/FANTOM/F5.hg38.enhancers.bed Permutated_intersected_enhancers
    ...
    parser.add_argument('-b', '--bed_path_dir', help='The directory containing the records to be converted.')
